...
parser.add_argument(
    '--lr-decay', type=float, default=0.1, help='learning rate ratio')
parser.add_argument(
    '--lr-decay-epoch',
    type=int,
    nargs='+',
...
parser.add_argument('--large-ratio', type=int, default=1, help='large ratio')
parser.add_argument(
...
for epoch in range(1, args.epochs + 1):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        hf_optm_pgb.progress_bar(
            batch_idx, len(train_loader), 'Loss: %.3f | Acc: %.3f%% (%d/%d)' %
            (train_loss / total_num, 100. * correct / total_num, correct,
             total_num))
    ...
    if args.eigen_type == 'full':
        ...
        eigenvalue, eigenvector = hf.get_eigen_full_dataset(
            model, test_loader, criterion, cuda=True, maxIter=10, tol=1e-2)
        ...
        eigenvalue, eigenvector = hf.get_eigen(
            model, inputs, targets, criterion, cuda=True, maxIter=10, tol=1e-2)
